...
-- ----------------------------
-- Updates made by Lind-it
-- ----------------------------

UPDATE `NL_USER` SET `NL_USER_PASSWORD`= AES_ENCRYPT('admin', 'aes_some_key_to_testdb777');

-- ----------------------------
-- Table structure for NL_HOUSES
-- ----------------------------
...
-- ----------------------------
INSERT INTO `NL_HOUSES` VALUES (1, 'Многоэтажный');
INSERT INTO `NL_HOUSES` VALUES (2, 'Коттедж');

-- ----------------------------
-- Table structure for NL_MATERIAL
-- ----------------------------
...
INSERT INTO `NL_MATERIAL` VALUES (2, 'Блок');
INSERT INTO `NL_MATERIAL` VALUES (3, 'Дерево');
INSERT INTO `NL_MATERIAL` VALUES (4, 'Газобетон');

-- ----------------------------
-- Update of NL_PROP_RESALE
-- ----------------------------

ALTER TABLE `NL_PROP_RESALE` ADD COLUMN `ID_NL_HOUSES` int(11) NULL;
ALTER TABLE `NL_PROP_RESALE` ADD CONSTRAINT `nl_prop_resale_ibfk_2` FOREIGN KEY(`ID_NL_HOUSES`) REFERENCES `NL_HOUSES`(`ID_NL_HOUSES`);

ALTER TABLE `NL_PROP_RESALE` ADD COLUMN `ID_NL_MATERIAL` int(11) NULL;
ALTER TABLE `NL_PROP_RESALE` ADD CONSTRAINT `nl_prop_resale_ibfk_3`  FOREIGN KEY(`ID_NL_MATERIAL`) REFERENCES `NL_MATERIAL`(`ID_NL_MATERIAL`);
